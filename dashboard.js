...
// Function to get department name in current language
function getDepartmentName(department) {
    return currentLanguage === 'tr' ? DEPARTMENTS[department] || department : department;
}

// Function to get department name in English
function getEnglishDepartmentName(turkishName) {
    return Object.entries(DEPARTMENTS).find(([eng, tr]) => tr === turkishName)?.[0] || turkishName;
}
...
async function sendMessage() {
    ...
    try {
        ...
        if (departmentMatch) {
            ...
            setTimeout(() => {
                addMessage(`I can help you schedule an appointment with the ${recommendedDepartment} department. Would you like to proceed?`, 'system');
...
function displayDepartmentOptions(departments) {
    ...
    departments.forEach(dept => {
        ...
        // Display department name in both languages
        const displayName = `${dept} / ${DEPARTMENTS[dept] || dept}`;
        deptButton.textContent = displayName;
...
async function loadPatientAppointments() {
    ...
    try {
        ...
        if (!USE_MOCK_DATA) {
            try {
                ...
                if (response.ok) {
                    ...
                    if (data && data.appointments) {
                        // API'den gelen randevuları Map'e ekle
                        data.appointments.forEach(apt => {
                            // API'den gelen randevuların ID'lerini koru
                            appointments.set(apt.id, {
                                ...apt,
                                appointment_date: apt.appointment_date // UTC ISO string olarak sakla
                            });
                        });
                    }
                }
            ...
            // Mock data durumunda sadece localStorage'ı kullan
            const localAppointments = await getAppointmentsFromLocalStorage();
            localAppointments.forEach(apt => {
                appointments.set(apt.id, apt);
            });
...
async function saveAppointmentToLocalStorage(appointmentData) {
    try {
        ...
        const duplicateTimeAppointment = storedAppointments.find(apt => {
            const aptTime = new Date(apt.appointment_date).getTime();
            return Math.abs(aptTime - newAppointmentTime) < 1000; // 1 saniyelik tolerans
        });
        
        if (duplicateTimeAppointment) {
            console.warn('Duplicate appointment time detected:', newAppointmentTime);
            return false;
        }
...
function updateDepartmentFilter() {
    ...
    let filterHTML = `<option value="all">${currentLanguage === 'tr' ? 'Tüm Bölümler' : 'All Departments'}</option>`;
    
    departments.forEach(department => {
        const displayName = getDepartmentName(department);
        filterHTML += `<option value="${department}">${displayName}</option>`;
    });
...
function displayAvailableTimeSlots(date) {
    ...
    timeSlots.forEach(time => {
        ...
        // UTC zamanını yerel saate çevirip göster
        const localTime = new Date(time.getTime() + new Date().getTimezoneOffset() * 60000);
        timeButton.textContent = formatTime(localTime);
        timeButton.dataset.utcTime = time.toISOString();
...
function finalizeAppointment(dateTime) {
    ...
    // Görüntüleme için yerel zamana çevir
    const localDateTime = new Date(utcDateTime.getTime() + new Date().getTimezoneOffset() * 60000);
    ...
    addMessage(`
        <div class="appointment-summary">
            ...
            <p><strong>Date & Time:</strong> ${formatDate(localDateTime)} at ${formatTime(localDateTime)}</p>
...
async function bookAppointment() {
    try {
        ...
        // Seçilen saatte randevu var mı kontrol et
        ...
        const sameTimeAppointment = existingAppointments.find(apt => {
            ...
            // Compare only the date and hour, ignore minutes and seconds
            return aptTime.getFullYear() === selectedTime.getFullYear() &&
                   aptTime.getMonth() === selectedTime.getMonth() &&
                   aptTime.getDate() === selectedTime.getDate() &&
                   aptTime.getHours() === selectedTime.getHours();
        });
        ...
        // Benzersiz bir randevu ID'si oluştur (timestamp + 4 haneli random sayı)
        const timestamp = Date.now();
        const randomNum = Math.floor(Math.random() * 10000).toString().padStart(4, '0');
        currentAppointmentId = parseInt(`${timestamp}${randomNum}`);
        ...
        const appointmentData = {
            ...
            appointment_date: appointmentDate.toISOString(), // UTC zaman
